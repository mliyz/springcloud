...
import com.liyz.fallInLove.auth.CurrentUser;
...
public abstract class BZBaseEntiy extends BaseEntity
{
    ...
    private static final String CREATOR_NAME = "exp_system";
    public BZBaseEntiy() {
        super();
    }
    ...
    public String create() {
        ...
        //如果已经手动设置了创建人则不需要走此处的数据设置
        if(isSMAvailable()&&CREATOR_NAME.equals(this.getCreatorName())){
            this.setCreatorId(CurrentUser.getUserId());
            this.setCreatorName(CurrentUser.getCnName()+"@"+CurrentUser.getUsername());
            this.setCreatorDepartmentName(CurrentUser.getDepartmentName());
        }
        this.save();
        return this.id;
    }
    ...
    public String update() {
        this.setUpdateTime(new Date());
        //如果已经手动设置了更新人则不需要走此处的数据设置
        if(isSMAvailable()&&this.getUpdatorName()==null){
            this.setUpdatorId(CurrentUser.getUserId());
            this.setUpdatorName(CurrentUser.getCnName()+"@"+CurrentUser.getUsername());
            this.setUpdatorDepartmentName(CurrentUser.getDepartmentName());
        }
        this.save();
        return this.id;
    }
    ...
    /**
     * 验证 org.apache.shiro.mgt.SecurityManager 是否可用
     * @return
     */
