package com.liyz.common.system.querychannel;
...
/**
 * 一个辅助类，处理查询语句，根据原始查询语句生成计算查询结果总数的查询语句 因为客户代码不会直接使用该类，所以设置为包级可见性。 Created by yyang on 14-2-25.
 */
class CountQueryStringBuilder {
	...
	public String buildQueryStringOfCount() {
		...
		StringBuilder builder = new StringBuilder(
				"select count(" + stringInCount(result, index) + ") ");
	...
	public String removeOrderByClause() {
		Matcher m = Pattern.compile("order\\s*by[\\w|\\W|\\s|\\S]*", Pattern.CASE_INSENSITIVE)
				.matcher(queryString);
	...
	private static String stringInCount(String queryString, int fromIndex) {
		...
		String strMayBeWithAs = commaIndex == -1 ? distinctToFrom
				: distinctToFrom.substring(0, commaIndex);
	...
	public boolean containsGroupByClause() {
		Matcher m = Pattern.compile("group\\s*by[\\w|\\W|\\s|\\S]*", Pattern.CASE_INSENSITIVE)
				.matcher(queryString);
		return m.find();
	}
}
